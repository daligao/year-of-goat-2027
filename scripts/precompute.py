#!/usr/bin/env python3
"""
Pre-compute heavy data once via Actions, serve as static JSON.
Generates: data/lunar-dates.json (CNY 1900-2100 + all festivals)
"""
import json
from datetime import date, timedelta

# Full CNY dates 1900-2030 (authoritative lookup)
CNY_RAW = {
...
def get_sign(year):
    return SIGNS[((year - 1900) % 12 + 12) % 12]

def get_element(year):
    stem = (year - 1900) % 10
    elements = ['Metal','Metal','Water','Water','Wood','Wood','Fire','Fire','Earth','Earth']
    yin_yang = ['Yang','Yin','Yang','Yin','Yang','Yin','Yang','Yin','Yang','Yin']
    return f"{yin_yang[stem]} {elements[stem]}"

output = {}
for year, (month, day) in CNY_RAW.items():
    cny_date = date(year, month, day)
    zodiac_year = year if date(year, month, day) <= cny_date else year - 1
    # Actually: if born before CNY, belongs to previous zodiac year
    output[str(year)] = {
        "cny": f"{year}-{month:02d}-{day:02d}",
        "lantern": str(cny_date + timedelta(days=14)),
        "sign": get_sign(year),
...
result = {
    "_source": "chinesefortunetools.online/data/lunar-dates.json",
    "_note": "Pre-computed by GitHub Actions. CNY dates 1900-2030 with zodiac/element.",
    "_updated": str(date.today()),
    "years": output
}

with open("data/lunar-dates.json", "w") as f:
    json.dump(result, f, separators=(',',':'))

print(f"Generated {len(output)} years → data/lunar-dates.json")
